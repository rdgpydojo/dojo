...
#  Agents send a message with they are attacked

#  Agents always attack

#  Agents goto the location of the attack, except for scouts that keep looking

#
#  Results:
#  Large growing swarm that explores that area for all plants as fast as possible
#  until the enemy is found.
#
#  By the time the enemy is found everyone is spread out
#  Once the enemy is found everyone heads in that direction and if there are any
#  plants between the two they are usually taken before the enemy.
#
...
class AgentMind(object):
    ...
    def __init__(self, args):
        ...
        self.is_scout = (random.random() > self.SCOUT_LEVEL)
    ...
    def act(self, view, msg):
        ...
        for agent in view.get_agents():
            if (agent.get_team() != me.get_team()):
                ...
                return cells.Action(cells.ACT_ATTACK, agent.get_pos())
        ...
        if (self.is_scout and me.energy > 1000 and random.random()>0.5):
            (spawn_x, spawn_y) = self.smart_spawn(me, view)
            return cells.Action(cells.ACT_SPAWN,(mx + spawn_x, my + spawn_y, self))
        ...
        if (self.step == 0 and True != self.is_scout and (random.random()>0.2)) :
            ax = 0;
            ay = 0;
